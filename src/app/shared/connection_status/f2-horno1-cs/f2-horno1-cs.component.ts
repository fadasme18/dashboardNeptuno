...
@Component({
  selector: 'app-f2-horno1-cs',
  templateUrl: './f2-horno1-cs.component.html',
  styleUrls: ['./f2-horno1-cs.component.scss']
})
export class F2Horno1CSComponent implements OnInit, OnDestroy {
  ...
  loadData() {
    this.influxdbService.getData_F2_horno1_energia().subscribe((data: any[]) => {
      ...
      data.forEach(item => {
        if (item._field === 'Energy') {          
          ...
          if (parseFloat(item._value) > 0) {
            status = 'Online'
          }else if (parseFloat(item._value) < 10){
            status = 'Offline' 
          }
        }
      });
