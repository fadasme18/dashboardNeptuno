...
@Component({
  selector: 'app-f2-horno2-corriente',
  templateUrl: './f2-horno2-corriente.component.html',
  styleUrls: ['./f2-horno2-corriente.component.scss']
})
export class F2Horno2CorrienteComponent implements OnInit, OnDestroy {
  ...
  async loadData() {
    ...
    if (this.startDate && this.endDate) {
      this.data = await this.InfluxdbService.F2_horno2_corriente(this.startDate, this.endDate);
    ...
    //---------------------------------------------------------------------------------------------------------------------
      ...
      this.data.forEach(item => {
        ...
        } else if (item._field === 'CurrentB') {
          this.currentData1.push({
            x: new Date(item._time).getTime(), // Convierte la fecha en una marca de tiempo
            y: Number(parseFloat(item._value).toFixed(2)) // Convierte el valor en número
          });
        } else if (item._field === 'CurrentC') {
      ...
      corriente2 = this.currentData;
      corriente3 = this.currentData;
  ...
  graficar() {
    this.chartOptions = {
      title: {
        text: 'Corriente Equipo 2 (Horno de inducción) Fundición 2',
        align: 'center'
      },
