...
@Component({
  selector: 'app-f2-silo-sur',
  templateUrl: './f2-silo-sur.component.html',
  styleUrls: ['./f2-silo-sur.component.scss']
})
export class F2SiloSurComponent implements OnInit, OnDestroy {
  ...
  async loadData() {
    ...
    if (this.startDate && this.endDate) {
      this.data = await this.InfluxdbService.F2_silo_sur(this.startDate, this.endDate);
  ...
  graficar() {
    ...
    this.chartOptions = {
      title: {
        text: 'Silo sur Fundición 2 (Arena Nueva)',
        align: 'center'
      },
