...
@Component({
  selector: 'app-f2-horno1-voltage',
  templateUrl: './f2-horno1-voltage.component.html',
  styleUrls: ['./f2-horno1-voltage.component.scss']
})
export class F2Horno1VoltageComponent implements OnInit, OnDestroy {
  ...
  async loadData() {
    ...
    if (this.startDate && this.endDate) {
      this.data = await this.InfluxdbService.F2_horno1_voltage(this.startDate, this.endDate);
    ...
    //---------------------------------------------------------------------------------------------------------------------
      ...
      this.data.forEach(item => {
        ...
        else if (item._field === 'Voltage2') {
          this.voltageData1.push({
            x: new Date(item._time).getTime(), // Convierte la fecha en una marca de tiempo
            y: Number(parseFloat(item._value).toFixed(2)) // Convierte el valor en número
          });
        }
        else if (item._field === 'Voltage3') {
  ...
  graficar() {
    ...
    this.chartOptions = {
      title: {
        text: 'Voltaje Equipo 1 (Horno de inducción) Fundición 2',
        align: 'center'
      },
